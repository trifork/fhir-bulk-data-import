...
* Post the Parameters file to HAPI FHIR at the `$import` endpoint and with the `HTTP Header` `Prefer: respond-async` with the Bulk Import feature enabled as well as the `auto_create_placeholder_reference_targets` set to `true`.
* Profit $$$
